...
def read_metadata(f: h5py.File) -> Metadata:
    ...
    if neuroscan_version < (0, 1, 3):
        raise NotImplementedError(f"files generated by IconScan earlier than 0.1.3 is not supported yet")
...
def read_scan(file) -> Scan:
    with h5py.File(file, 'r') as f:
        ...
        if not np.all(np.abs(d) == voxel_size):
            raise ValueError(f"voxel size {voxel_size} doesn't match voxels to probe transform {d}")

        check_valid_transform(probe_to_lab, batch_shape=(4,))
